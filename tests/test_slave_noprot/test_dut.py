...
async def test_apb_pprot(dut):
    ...
    #     except :
    #         pass
    await tb.m.read(0x1000, 0x52346325, prot=ApbProt.NONSECURE, error_expected=True)

    await tb.m.write(0x2000, 0xA356B3E1, prot=ApbProt.INSTRUCTION)
    await tb.m.read(0x2000, 0xA356B3E1, prot=ApbProt.INSTRUCTION)
    await tb.m.read(0x2000, 0xA356B3E1, prot=ApbProt.NONSECURE)
    await tb.m.read(0x2000, 0xA356B3E1, prot=ApbProt.NONSECURE, error_expected=True)
